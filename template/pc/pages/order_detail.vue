<template>
  <div class="order-detail wrapper_1200">
    ...
    <div class="section process bag-f" v-if="orderData._status._type >= 0">
      <div class="section-hd" v-if="orderData.paid === 0">
        订单状态：{{ orderData.is_cancel === 0 ? "待付款" : "已取消" }}
      </div>
      <div class="section-hd" v-if="orderData._status._type === 1 && orderData.paid === 1">
        订单状态：待发货
      </div>
      <div class="section-hd" v-if="orderData._status._type === 2 && orderData.paid === 1">
        订单状态：待收货
      </div>
      <div class="section-hd" v-if="orderData._status._type === 3 && orderData.paid === 1">
        订单状态：待评价
      </div>
      <div class="section-hd" v-if="orderData._status._type === 4 && orderData.paid === 1">
        订单状态：已完成
      </div>
      <div class="section-bd">
        <ul class="">
          <li
            :class="{
              past:
                orderData._status.paid > 0 && orderData.shipping_type === 1,
              now:
                orderData._status.paid === 0 && orderData.shipping_type === 1,
            }"
          >
            ...
            <div class="info">
              <div>{{ orderData.is_cancel === 0 ? "待付款" : "已取消" }}</div>
          ...
          <li
            :class="{
              past:
                orderData._status._type > 1 && orderData.shipping_type === 1 && orderData.paid === 1,
              now:
                orderData._status._type === 1 && orderData.shipping_type === 1 && orderData.paid === 1,
            }"
          >
            ...
            <div
              :class="[
                'iconfont',
                orderData._status._type > 0 && orderData.shipping_type === 1 && orderData.paid === 1
                  ? 'icon-xuanzhong11'
                  : 'icon-weixuan',
              ]"
            ></div>
          ...
          <li
            :class="{
              past:
                orderData._status._type > 2 && orderData.shipping_type === 1 && orderData.paid === 1,
              now:
                orderData._status._type === 2 && orderData.shipping_type === 1 && orderData.paid === 1,
            }"
          >
          ...
          <li
            :class="{
              past:
                orderData._status._type > 3 && orderData.shipping_type === 1 && orderData.paid === 1,
              now:
                orderData._status._type === 3 && orderData.shipping_type === 1 && orderData.paid === 1,
            }"
          >
          ...
          <li
            :class="{
              past:
                orderData._status._type > 4 && orderData.shipping_type === 1 && orderData.paid === 1,
              now:
                orderData._status._type === 4 && orderData.shipping_type === 1 && orderData.paid === 1,
            }"
          >
    ...
    <div
      v-if="orderData.refund_type == 3 && orderData.refuse_reason"
    ...
    <div class="section bag-f" v-if="[4, 5].includes(orderData.refund_type)">
      <div class="section-hd">退货信息</div>
      <div class="section-bd" v-if="orderData._status.refund_name != ''">
        <ul>
          <li class="acea-row row-middle">
            <div>退货姓名：</div>
            <div>{{ orderData._status.refund_name }}</div>
          </li>
          <li class="acea-row row-middle">
            <div>退货电话：</div>
            <div>{{ orderData._status.refund_phone }}</div>
          </li>
          <li class="acea-row row-middle">
            <div>退货地址：</div>
            <div>{{ orderData._status.refund_address }}</div>
          </li>
          <li class="acea-row row-middle">
            <span style="color: #e93323;font-size: 12px;">请按以上退货信息将商品退回</span>
          </li>
        </ul>
      </div>
      <div class="section-bd" v-else>
        <ul>
          <li class="acea-row row-middle">
            <span style="color: #e93323;">请联系管理员获取退货地址</span>
          </li>
        </ul>
      </div>
    </div>
    <div class="section bag-f">
      ...
      <div class="section-bd">
        <ul>
          ...
          <li class="acea-row row-middle">
            <div>支付状态：</div>
            <div>{{ orderData.is_cancel === 0 ? orderData.paid === 0 ? "未支付" : "已支付" : "已取消" }}</div>
          </li>
          <li class="acea-row row-middle">
            <div>支付方式：</div>
            <div>{{ orderData.is_cancel === 0 ? orderData._status._payType : "" }}</div>
          </li>
    ...
    <div class="section bag-f" v-if="orderData.virtual_type == 0">
      <div class="section-hd">收货信息</div>
      <div class="section-bd">
        <ul>
          <li class="acea-row row-middle" v-if="orderData.type == 4">
            <div>预售发货：</div>
            <div>{{ orderData._status._msg }}</div>
          </li>
          <li class="acea-row row-middle">
            <div>收货人：</div>
    ...
    <div class="section bag-f">
      ...
      <div class="section-bd">
          ...
          >
            ...
            <div>
              ...
              > -->
                  ...
                  <div class="refund" v-if="item.refund_num">
        ...
        <ul>
          ...
          <li class="acea-row row-middle" v-if="orderData.levelPrice != '0.00'">
            <div>等级折扣：</div>
          ...
          <li
            class="acea-row row-middle"
            v-if="parseFloat(orderData.coupon_price) > 0"
          >
            <div>优惠券抵扣：</div>
            <div>-￥{{ orderData.coupon_price }}</div>
          </li>
        </ul>
        <ul class="total">
