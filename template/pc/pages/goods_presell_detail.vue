            ...
            <div class="acea-row row-middle money-wrapper">
              <div class="money-wrap">
                ...
                <span class="start-time" v-if="presale_pay_status === 1">
                  开售时间: {{ storeInfo.presale_start_time }}
                </span>
                <span class="start-time" v-if="presale_pay_status === 2">
                  活动时间: {{ storeInfo.presale_start_time }}~{{
                    storeInfo.presale_end_time
                  }}
                </span>
                <div>预售结束后{{storeInfo.presale_day}}天内发货</div>
              </div>
...
export default {
  ...
  async asyncData({ app, query }) {
    const [detail, config, list, goodsList, phoneBuy, coupon] =
      await Promise.all([
