  ...
  <div class="order_confirm wrapper_1200">
    ...
    <div class="wrapper wrapper_1200">
        ...
        <div class="coupon" v-if="!deduction">
          ...
          <div class="acea-row row-middle" v-if="parseFloat(userInfo.integral)">
            ...
            <div class="integralCurrent">
              当前积分<span class="num font-color">{{ integral }}</span>
            </div>
          </div>
        </div>
        <!-- confirm -->
      ...
      <div class="totalCon">
        ...
        <div class="total acea-row row-middle row-right" v-if="freight > 0">
          <div>运费：</div>
          <div class="money">¥{{ freight || 0 }}</div>
        </div>
        <div
          class="total acea-row row-middle row-right"
          v-if="priceGroup.storePostageDiscount > 0"
        >
          <div>会员运费优惠：</div>
        ...
        >
          <div>等级折扣：</div>
          <div class="money">- ¥{{ priceGroup.levelPrice || 0 }}</div>
        </div>
        <div
          class="total acea-row row-middle row-right"
        ...
        >
          <div>会员折扣：</div>
          <div class="money">- ¥{{ priceGroup.memberPrice || 0 }}</div>
        </div>
      </div>
...
export default {
  ...
  async asyncData({ app, query }) {
    ...
    return {
      ...
      usable_integral: confirmMsg.data.usable_integral,
      integral: confirmMsg.data.usable_integral,
      ...
      freight:
        parseFloat(confirmMsg.data.priceGroup.storePostage) +
        parseFloat(confirmMsg.data.priceGroup.storePostageDiscount),
      news: query.new ? 1 : 0,
  ...
  methods: {
    ...
    computedPrice() {
      this.$axios
        ...
        .then((res) => {
          ...
          this.integral = this.useIntegral
            ? this.computeData.SurplusIntegral
            : this.usable_integral;
          this.freight =
            parseFloat(res.data.result.pay_postage) +
            parseFloat(res.data.result.storePostageDiscount);
        })
        .catch((err) => {
          Message.error(err);
        });
    ...
    orderConfirm() {
      this.$axios
        ...
        .then((res) => {
          let confirmMsg = res.data;
          this.cartInfo = confirmMsg.cartInfo;
          this.confirm = confirmMsg.custom_form || [];
          this.userInfo = confirmMsg.userInfo;
          this.usable_integral = confirmMsg.usable_integral;
          this.integral = confirmMsg.usable_integral;
          this.deduction = confirmMsg.deduction;
          this.virtual_type = confirmMsg.virtual_type;
          this.orderKey = confirmMsg.orderKey;
          this.priceGroup = confirmMsg.priceGroup;
          this.totalPrice = confirmMsg.priceGroup.totalPrice;
          this.seckillId = confirmMsg.seckill_id;
          this.freight =
            parseFloat(confirmMsg.priceGroup.storePostage) +
            parseFloat(confirmMsg.priceGroup.storePostageDiscount);
          this.validCount = confirmMsg.valid_count;
        });
    },
    open() {
      this.isShow = true;
