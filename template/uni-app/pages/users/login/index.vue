<template>
	<view class="login-wrapper" :style="colorStyle">
		...
		<view class="whiteBg" v-if="formItem === 1 || formItem === 2">
			<view v-if="formItem === 1">
				...
				<!-- 手机验证码登录/绑定手机号 -->
				<view class="list" v-if="current === 1 || appLoginStatus || appleLoginStatus">
					<view class="item">
						<view class="acea-row row-middle">
							<image src="../static/phone_1.png" style="width: 24rpx; height: 34rpx"></image>
				...
				<view class="list" v-if="current === 2">
					<view class="item">
						<view class="acea-row row-middle">
							<image src="../static/phone_1.png" style="width: 24rpx; height: 34rpx"></image>
							<input type="text" :placeholder="$t(`输入邮箱`)" v-model="email" />
						</view>
					</view>
					<view class="item">
						<view class="acea-row row-middle">
				...
				<view class="logon" @click="loginMobile" v-if="current === 1">{{ $t(`登录`) }}</view>
				<view class="logon" @click="loginEmail" v-if="current === 2">{{ $t(`登录`) }}</view>

				<!-- #ifndef APP-PLUS -->
				<view class="login-switch">
					<view class="switch-item" v-if="current !== 2" @click="current = 2">{{ $t(`邮箱登录`) }}</view>
					<text class="switch-divider" v-if="current !== 2 && current !== 1">|</text>
					<view class="switch-item" v-if="current !== 1" @click="current = 1">{{ $t(`手机登录`) }}</view>
					<text class="switch-divider" v-if="current !== 0">|</text>
					<view class="switch-item" v-if="current !== 0" @click="current = 0">{{ $t(`账号登录`) }}</view>
				</view>
				<view class="register-link">
					<text class="register-text">{{ $t(`还没有账号？`) }}</text>
					<text class="register-btn" @click="formItem = 2">{{ $t(`立即注册`) }}</text>
				</view>
				<!-- #endif -->

				<!-- #ifdef APP-PLUS -->
				<view class="appLogin" v-if="!appLoginStatus && !appleLoginStatus && current !== 2">
					<view class="hds">
				...
				<view v-else class="list">
					<view class="item" v-if="registerVerifyType === 'phone'">
						<view class="acea-row row-middle">
							<image src="../static/phone_1.png" style="width: 24rpx; height: 34rpx"></image>
							<input type="text" :placeholder="$t(`输入手机号码`)" v-model="account" maxlength="11" />
						</view>
					...
					<view class="item verify-switch">
						<view class="switch-item" :class="{ on: registerVerifyType === 'phone' }" @click="registerVerifyType = 'phone'">
							{{ $t(`手机验证码`) }}
						</view>
						<text class="switch-divider">|</text>
						<view class="switch-item" :class="{ on: registerVerifyType === 'email' }" @click="registerVerifyType = 'email'">
							{{ $t(`邮箱验证码`) }}
						</view>
					</view>
					<view class="item">
					...
					<view class="item">
						<view class="acea-row row-middle">
							<image src="../static/phone_1.png" style="width: 24rpx; height: 34rpx"></image>
							<input type="text" :placeholder="$t(`填写昵称`)" v-model="nickname" />
					</view>
				</view>
				<view class="item">
					<view class="acea-row row-middle">
						<image src="../static/phone_1.png" style="width: 24rpx; height: 34rpx"></image>
						<input type="text" :placeholder="$t(`填写真实姓名`)" v-model="real_name" />
					</view>
				</view>
					<view class="item">
						<picker :range="sexRange" @change="onSexChange">
							<view class="acea-row row-middle">
								<image src="../static/phone_1.png" style="width: 24rpx; height: 34rpx"></image>
								<view class="picker-text" :class="{ 'is-placeholder': sex === '' || sex === null || sex === undefined }">{{ sexText }}</view>
							</view>
						</picker>
					</view>
				<view class="item">
				...
				<!-- #ifndef APP-PLUS -->
				<view class="tips">
					<view @click="formItem = 1">{{ $t(`返回登录`) }}</view>
				</view>
				<!-- #endif -->
			</view>
...
<script>
	...
	import { loginH5, loginMobile, loginEmail as loginEmailApi, registerVerify, emailVerify as emailVerifyApi, register, getCodeApi, getUserInfo, appleLogin } from '@/api/user';
...
export default {
	...
	mixins: [sendVerifyCode, colors],
		data: function () {
			return {
				...
				password: '',
					...
					nickname: '',
						real_name: '',
						sex: '',
						sexRange: [this.$t(`保密`), this.$t(`男`), this.$t(`女`)],
			...
			computed: {
				sexText() {
					if (this.sex === '' || this.sex === null || this.sex === undefined) return this.$t(`请选择性别`);
					return this.sexRange[this.sex] ? this.sexRange[this.sex] : this.sexRange[0];
				}
			},
		watch: {
	...
	onLoad() {
		...
		methods: {
			onSexChange(e) {
				this.sex = Number(e.detail.value) || 0;
			},
			ChangeIsDefault(e) {
		...
		success(data) {
			...
			getCodeApi()
				...
				code() {
					...
					if ((that.formItem === 1 && that.current === 2) || (that.formItem === 2 && that.registerVerifyType === 'email')) {
						if (!that.email)
							return that.$util.Tips({
								title: that.$t(`请填写邮箱`)
							});
		...
		async loginMobile() {
				...
				if (!that.real_name)
					return that.$util.Tips({
						...
						register({
							...
							sex: that.sex === '' || that.sex === null || that.sex === undefined ? 0 : that.sex,
			...
			async getCode(data) {
				let that = this;
					...
					if (that.formItem === 2 && that.registerVerifyType === 'email') {
					...
					if (that.formItem === 1 && that.current === 2) {
						await emailVerifyApi({
							email: that.email,
							type: that.type,
						key: that.keyCode,
